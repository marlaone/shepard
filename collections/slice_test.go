...
func TestSlice_Capacity(t *testing.T) {
	s := collections.WithCapacity[int](5)
...
func TestSlice_Truncate(t *testing.T) {
	...
	s.Truncate(2)
	assert.Equal(t, collections.Init(1, 2), s)
}

func TestSlice_Insert(t *testing.T) {
	s := collections.Init(1, 2, 3)
	s.Insert(1, 4)
	assert.Equal(t, collections.Init(1, 4, 2, 3), s)
	s.Insert(4, 5)
	assert.Equal(t, collections.Init(1, 4, 2, 3, 5), s)
}

func TestSlice_Remove(t *testing.T) {
	s := collections.Init(1, 2, 3)
	assert.Equal(t, 2, s.Remove(1))
	assert.Equal(t, collections.Init(1, 3), s)
}

func TestSlice_Retain(t *testing.T) {
	s := collections.Init(1, 2, 3, 4)
	s.Retain(func(x *int) bool { return *x%2 == 0 })
	assert.Equal(t, collections.Init(2, 4), s)
}

func TestSlice_Push(t *testing.T) {
	s := collections.Init(1, 2)
	s.Push(3)
	assert.Equal(t, collections.Init(1, 2, 3), s)
}
...
func TestSlice_Append(t *testing.T) {
	s := collections.Init(1, 2, 3)
	s2 := collections.Init(4, 5, 6)
	s.Append(&s2)

	assert.Equal(t, collections.Init(1, 2, 3, 4, 5, 6), s)
	assert.Equal(t, collections.New[int](), s2)
}
...
func TestSlice_Swap(t *testing.T) {
	v := collections.Init("a", "b", "c", "d", "e")
	v.Swap(2, 4)
	assert.Equal(t, collections.Init("a", "b", "e", "d", "c"), v)
}

func TestSlice_Reverse(t *testing.T) {
	v := collections.Init(1, 2, 3)
	v.Reverse()
	assert.Equal(t, collections.Init(3, 2, 1), v)
}

func TestSlice_Iter(t *testing.T) {
	x := collections.Init(1, 2, 4)
	iter := x.Iter()
